...
export class SupabaseDataService {
  constructor(snapAuthorization, devConfig, model) {
    this.model = model;
    ...
    // No platform signal exists; watch MetaInfo/project changes and emit only when SPECS flips.
    this._isSpecs = this.isSpecsPlatform();
    const rerenderPlatform = () => {
      try {
        const isSpecs = this.isSpecsPlatform();
        if (isSpecs === this._isSpecs) return;
        this._isSpecs = isSpecs;
        this.emit('PlatformChanged', { isSpecs });
      } catch (e) {
        console.error('[Supabase] PlatformChanged detection failed:', e, console.None);
      }
    };
    if (this.model) {
      if (this.model.onMetaInfoChanged) this.signals.push(this.model.onMetaInfoChanged.connect(rerenderPlatform));
      if (this.model.onProjectChanged) this.signals.push(this.model.onProjectChanged.connect(rerenderPlatform));
    }
  }

  // Snap Cloud is SPECS-only. False if the platform isn't known yet (project not loaded).
  isSpecsPlatform() {
    // A destroyed native project is still truthy in JS; guard with Editor.isNull().
    const project = this.model ? this.model.project : null;
    if (!project || Editor.isNull(project)) return false;
    return project.targetPlatform === Editor.TargetPlatform.Spectacles;
  }
  ...
  requestSupabaseAuthorization() {
    ...
    // Snap Cloud is SPECS-only: never exchange on other platforms (guards against global
    // SnapAuthorization events, e.g. background token refreshes, firing this on a non-SPECS project).
    if (!this.isSpecsPlatform()) {
      this.emitFailure('SupabaseAuthorization', 'Snap Cloud is available on SPECS projects only');
      return;
    }
  ...
  fetchProjectInfo(projectIndex) {
    ...
    reply.onEnd.connect((httpResult) => {
      ...
      // Extract anon and service_role keys (supporting both legacy and new formats)
      let anonToken = '';
      let publishableToken = '';

      this.projectKeys.forEach(key => {
        // Handle legacy format (id === 'anon')
        if (key.id === 'anon') {
          anonToken = key.api_key;
        }
        // Handle new format (type === 'publishable')
        else if (key.type === 'publishable') {
          publishableToken = key.api_key;
        }
      });
      ...
      credential.publishableToken = publishableToken;
      ...
      this.emit('projectInfoFetched', {
        success: true,
        project: project,
        publicToken: publishableToken || anonToken,
  ...
  clearAllData() {
    ...
    this.managementApiToken = null;
    this.statusMessage = "";
    // Require an explicit Login click after logout.
    this.agreeToTerms = false;
  }

  // Cleanup method for complete shutdown
  cleanup() {
    this.signals.forEach((s) => s && s.disconnect && s.disconnect());
    this.signals = [];
    this.clearAllData();
    this.eventListeners = {};
  }
