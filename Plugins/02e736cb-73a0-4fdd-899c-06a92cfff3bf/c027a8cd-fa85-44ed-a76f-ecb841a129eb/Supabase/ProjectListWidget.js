...
export class ProjectListWidget {
  ...
  setupDataServiceListeners() {
    ...
    this.dataService.addEventListener('projectInfoFetched', (data) => {
      if (data.success && data.publicToken!=='') {
  ...
  importCredentialsForProject(project) {
    ...
    const publishableTokenDeployWarning = 'Disable "Verify JWT with legacy secret" while deploying edge functions.';

    if (credential && credential.publicToken !== '') {
      ...
      for (const existing of existingSupabaseProject) {
        if (existing.projectId === credential.id) {
          ...
          existing.publicToken = credential.publicToken;
          if (credential.hasPublishableToken) {
            console.log(`[Supabase] ${publishableTokenDeployWarning} projectId=${credential.id} projectName=${credential.name}`);
            this.dataService.updateStatus(`🟢 Credentials updated in ${existing.name}. ${publishableTokenDeployWarning}`);
          } else {
      ...
      supabaseProjectAsset.publicToken = credential.publicToken;
      if (credential.hasPublishableToken) {
        console.log(`[Supabase] ${publishableTokenDeployWarning} projectId=${credential.id} projectName=${credential.name}`);
        this.dataService.updateStatus(`🟢 Credentials imported in ${supabaseProjectAsset.name}. ${publishableTokenDeployWarning}`);
      } else {
