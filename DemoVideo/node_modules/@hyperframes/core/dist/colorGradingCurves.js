...
function endpointSlope(firstSpan, secondSpan, firstSlope, secondSlope) {
    const slope = ((2 * firstSpan + secondSpan) * firstSlope - firstSpan * secondSlope) /
        (firstSpan + secondSpan);
    if (Math.sign(slope) !== Math.sign(firstSlope))
        return 0;
...
function valueAt(values, index) {
    ...
    if (value === undefined)
        throw new RangeError("Color curve points must be contiguous");
    return value;
}
function pointSlopes(points) {
    const spans = [];
    const slopes = [];
    ...
    if (points.length === 2)
        return [firstSlope, firstSlope];
    const result = new Array(points.length);
    result[0] = endpointSlope(valueAt(spans, 0), valueAt(spans, 1), firstSlope, valueAt(slopes, 1));
    result[result.length - 1] = endpointSlope(valueAt(spans, spans.length - 1), valueAt(spans, spans.length - 2), valueAt(slopes, slopes.length - 1), valueAt(slopes, slopes.length - 2));
...
function validateCurvePoints(points, size) {
    if (points.length < 2)
        throw new RangeError("A color curve requires at least two points");
...
function validateOutputRange(points, outputMin, outputMax) {
...
function interpolateCurveSegment(input, start, end, startTangent, endTangent) {
    const span = end[0] - start[0];
    const t = Math.min(1, Math.max(0, (input - start[0]) / span));
    const t2 = t * t;
    const t3 = t2 * t;
    return ((2 * t3 - 3 * t2 + 1) * start[1] +
        (t3 - 2 * t2 + t) * span * startTangent +
        (-2 * t3 + 3 * t2) * end[1] +
        (t3 - t2) * span * endTangent);
}
function compileCurveSamples(points, size, inputAt, outputMin, outputMax) {
    ...
    for (let index = 0; index < size; index += 1) {
        ...
        const output = interpolateCurveSegment(input, start, end, valueAt(tangents, segment), valueAt(tangents, segment + 1));
...
export function compileHfColorCurve(points, size = HF_COLOR_CURVE_LUT_SIZE) {
...
export function compileHfHueCurve(points, outputMin, outputMax, size = HF_COLOR_CURVE_LUT_SIZE) {
    if (points.length < 3)
        throw new RangeError("A hue curve requires at least three points");
    if (points.length > HF_COLOR_CURVE_MAX_POINTS) {
    ...
    const before = sorted.slice(-2).map(([hue, delta]) => [hue - 360, delta]);
    const after = sorted.slice(0, 2).map(([hue, delta]) => [hue + 360, delta]);
...
//# sourceMappingURL=colorGradingCurves.js.map