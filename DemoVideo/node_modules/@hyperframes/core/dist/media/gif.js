...
function colorTableByteLength(packed) {
...
function parseApplicationExtension(bytes, offset) {
    const blockSize = bytes[offset];
    if (blockSize === undefined || offset + 1 + blockSize > bytes.length)
        return null;
    ...
    while (pos < bytes.length) {
        ...
        pos += 1;
        if (size === 0)
            return { nextOffset: pos, loopCount };
...
export function parseAnimatedGifMetadata(bytes) {
    if (bytes.length < 13)
        return null;
    ...
    while (pos < bytes.length) {
        ...
        if (introducer === 0x21) {
            ...
            if (label === 0xf9) {
                ...
                if (blockSize !== 4 || pos + 6 > bytes.length)
                    return null;
