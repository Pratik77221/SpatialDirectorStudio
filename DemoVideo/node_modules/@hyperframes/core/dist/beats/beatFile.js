...
export function audioRelPathForSrc(src) {
    if (!src)
        return null;
    ...
    if (previewIdx !== -1) {
        ...
        if (queryOrHash !== -1)
            after = after.slice(0, queryOrHash);
        if (after.startsWith("comp/"))
            after = after.slice("comp/".length);
...
export function parseBeats(content) {
    try {
        ...
        if (!data || data.version !== 1 || !Array.isArray(data.beats))
            return null;
        const times = [];
        const strengths = [];
 ...
 */
export function findMusicAudioSrc(html) {
    ...
    for (const tag of tags) {
        ...
        if (id && MUSIC_ID_RE.test(id))
            return src;
    }
    return null;
}
//# sourceMappingURL=beatFile.js.map