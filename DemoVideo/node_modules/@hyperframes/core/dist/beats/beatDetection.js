...
export function computeRmsAt(channelData, sampleRate, time) {
...
export function strengthAtTime(analysis, time) {
    if (!analysis.channelData || analysis.peak <= 0)
        return 0.5;
...
function regularizeBeats(rawBeats, bpm, duration) {
    if (rawBeats.length === 0 || bpm <= 0 || duration <= 0)
        return rawBeats;
...
function gateBeatsBySilence(beats, channelData, sampleRate) {
    if (beats.length === 0)
        return { times: beats, strengths: [], peak: 1e-6 };
...
// fallow-ignore-next-line complexity
export async function analyzeMusicFromBuffer(audioBuffer) {
    const channelData = audioBuffer.getChannelData(0);
    const sampleRate = audioBuffer.sampleRate;
    ...
    try {
        ...
        if (detect)
            detectiveBpm = detect(audioBuffer);
    }
    catch {
    ...
    if (onsetBpm !== null && detectiveBpm !== null) {
        const pctDiff = Math.abs(canonicalizeBpm(onsetBpm) - canonicalizeBpm(detectiveBpm)) /
...
export async function analyzeMusicFromUrl(url) {
    const audioContext = new AudioContext();
    try {
        const response = await fetch(url);
        const arrayBuffer = await response.arrayBuffer();
...
//# sourceMappingURL=beatDetection.js.map