...
export function isAllowedHtmlAttribute(name) {
    const lower = name.toLowerCase();
    if (lower.startsWith("on"))
        return false;
    if (ALLOWED_HTML_ATTRS.has(lower))
        return true;
...
export function isSafeAttributeValue(name, value) {
    if (URI_BEARING_ATTRS.has(name.toLowerCase())) {
        const trimmed = value.trim();
        if (DANGEROUS_URI_SCHEMES.test(trimmed))
            return false;
        if (DANGEROUS_DATA_URI.test(trimmed))
            return false;
    }
    return true;
}
//# sourceMappingURL=htmlAttrSafety.js.map