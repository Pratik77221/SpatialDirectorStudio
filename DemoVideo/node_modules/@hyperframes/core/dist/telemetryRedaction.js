 ...
 */
const ABSOLUTE_PATH = new RegExp(String.raw `(?<![:\w/\\])(?:[A-Za-z]:)?(?:[\\/]${SEGMENT}){2,}${TOKEN_TAIL}`, "g");
/** `./assets/bgm.mp3`, `../out.wav`, `.\tmp\x` — relative paths leak the same
 *  project structure absolute ones do, and were previously untouched. */
const RELATIVE_PATH = new RegExp(String.raw `(?<![\w/\\.])\.{1,2}(?:[\\/]${SEGMENT})+${TOKEN_TAIL}`, "g");
/**
 ...
 */
const ASSET_BASENAME = /(?<![\w/\\])[^\s/\\'"]+\.(?:mp4|mov|mkv|webm|avi|m4v|mpe?g|ts|mp3|wav|aac|m4a|flac|ogg|opus|png|jpe?g|gif|webp|svg|html?|json|srt|vtt|ass)\b/gi;
/**
 ...
 * reports only that.
 */
export function redactKnownPaths(value, paths) {
...
export function redactTelemetryString(value, maxLength = MAX_TELEMETRY_STRING_LENGTH) {
    return truncateTelemetryString(redactFilePaths(redactUrlQueryStrings(value)), maxLength);
}
//# sourceMappingURL=telemetryRedaction.js.map