...
export function audioFxWorkletsReady(ctx) {
 ...
 */
export function ensureAudioFxWorklets(ctx) {
    let modulePromise = registered.get(ctx);
    if (!modulePromise) {
        modulePromise = (async () => {
            if (!ctx.audioWorklet) {
                throw new Error("AudioWorklet is unavailable — the page needs a secure context (https, localhost or file://)");
            }
            ...
            const url = `data:text/javascript;base64,${btoa(String.fromCharCode(...new TextEncoder().encode(AUDIO_FX_WORKLET_SOURCE)))}`;
...
//# sourceMappingURL=audioFxWorklets.js.map