...
export function stripEmbeddedRuntimeScripts(html) {
    if (!html)
        return html;
...
function findScriptStart(loweredHtml, from) {
...
function findScriptCloseTagEnd(loweredHtml, from) {
    let index = loweredHtml.indexOf("</script", from);
    while (index !== -1) {
...
function findScriptCloseTagBoundary(loweredHtml, from) {
    let cursor = from;
...
function shouldStripRuntimeScriptBlock(block) {
...
function getScriptSource(block) {
...
function escapeInlineScriptSource(source) {
    return escapeCaseInsensitiveToken(escapeCaseInsensitiveToken(source, "</script", "<\\/script"), "<!--", "<\\!--");
}
function escapeCaseInsensitiveToken(source, token, replacement) {
...
function inlineScriptTags(scripts) {
 ...
 */
export function injectTagsAtHeadStart(html, tags) {
...
export function injectScriptsAtHeadStart(html, scripts) {
    if (scripts.length === 0)
        return html;
    return injectTagsAtHeadStart(html, inlineScriptTags(scripts));
}
export function injectScriptsIntoHtml(html, headScripts, bodyScripts, stripEmbeddedRuntime = true) {
