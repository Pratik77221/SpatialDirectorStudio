...
import { buildVariablesByCompScript, scopeCssToComposition, wrapInlineScriptWithErrorBoundary, wrapScopedCompositionScript, } from "./compositionScoping.js";
...
function injectInterceptor(html, runtimeMode = "inline") {
    const sanitized = stripEmbeddedRuntimeScripts(html);
    if (sanitized.includes(RUNTIME_BOOTSTRAP_ATTR))
        return sanitized;
...
const CSS_IMPORT_RE = /@import\s+(?:url\(\s*(["']?)([^)"']+)\1\s*\)|(["'])([^"']+)\3)\s*([^;]*);\s*/g;
...
function rebaseCssUrls(css, cssFileDir, projectDir) {
    ...
    if (resolvedDir === resolvedRoot)
        return css;
    return css.replace(CSS_URL_RE, (full, quote, urlValue) => {
        if (!urlValue || !isRelativeUrl(urlValue))
            return full;
...
function rebaseEntryAuthoredAssetPaths(document, sourceDir, projectDir) {
...
function inlineCssFile(css, cssFileDir, projectDir, visited = new Set()) {
    const { result: strippedCss, restore: restoreComments } = withCommentsStripped(css, (s) => s);
    const importPlaceholders = [];
    const withPlaceholders = strippedCss.replace(CSS_IMPORT_RE, (full, _q1, urlPath, _q2, barePath, mediaQuery) => {
        const importPath = urlPath ?? barePath;
        if (!importPath || !isRelativeUrl(importPath))
            return full;
        ...
        if (!isSafePath(projectDir, resolved))
            return full;
...
function appendSuffixToUrl(baseUrl, suffix) {
    if (!suffix)
        return baseUrl;
...
function maybeInlineRelativeAssetUrl(urlValue, projectDir) {
    if (!urlValue || !isRelativeUrl(urlValue))
        return null;
    const { basePath, suffix } = splitUrlSuffix(urlValue.trim());
    if (!basePath)
...
function isExternalSvgFragmentUse(el, attr, urlValue) {
    if (el.tagName.toLowerCase() !== "use")
        return false;
    if (attr !== "href" && attr !== "xlink:href")
        return false;
    if (!isRelativeUrl(urlValue))
        return false;
    const hashIdx = urlValue.indexOf("#");
    if (hashIdx <= 0)
        return false;
...
function warnColorGradingLutNotInlined(lutSrc) {
    const trimmed = lutSrc.trim();
    if (!isRelativeUrl(trimmed))
        return;
    console.warn(`[HyperFrames] Could not inline color grading LUT "${trimmed}". The rendered bundle may not be self-contained.`);
}
// fallow-ignore-next-line complexity
function rewriteColorGradingLutWithInlinedAssets(value, projectDir) {
    if (!value.trim().startsWith("{"))
        return value;
    let parsed;
    try {
...
function rewriteSrcsetWithInlinedAssets(srcsetValue, projectDir) {
    if (!srcsetValue)
        return srcsetValue;
    return srcsetValue
        .split(",")
        ...
        if (maybeInlined)
            parts[0] = maybeInlined;
        return parts.join(" ");
    })
        .join(", ");
}
function rewriteCssUrlsWithInlinedAssets(cssText, projectDir) {
    if (!cssText)
        return cssText;
    return cssText.replace(/\burl\(\s*(["']?)([^)"']+)\1\s*\)/g, (_full, quote, rawUrl) => {
...
function getBundledHostCompositionIdentity(host) {
    ...
    const authoredCompositionId = (host.getAttribute("data-hf-original-composition-id") || currentCompositionId || "").trim() ||
        null;
    return {
...
function getBundledTrackedCompositionHosts(document) {
    const hosts = Array.from(document.querySelectorAll("[data-composition-src], [data-composition-id]"));
    return hosts.filter((host) => {
        if (host.hasAttribute("data-composition-src"))
            return true;
...
function shouldAssignBundledRuntimeCompositionId(host, document) {
    if (host.hasAttribute("data-composition-src"))
        return true;
    const authoredCompositionId = getBundledHostCompositionIdentity(host).authoredCompositionId;
    if (!authoredCompositionId)
        return false;
    if (!document.getElementById(`${authoredCompositionId}-template`))
        return false;
...
// fallow-ignore-next-line complexity
export function assignBundledRuntimeCompositionIds(hosts, counts = countBundledAuthoredCompositionIds(hosts)) {
...
export function prepareFlattenedInnerRoot(innerRoot) {
...
function coalesceHeadStylesAndBodyScripts(document) {
    ...
    const bodyInlineScripts = [...document.querySelectorAll("body script")].filter((el) => {
        if (el.hasAttribute(RUNTIME_BOOTSTRAP_ATTR) || el.hasAttribute("src"))
            return false;
 ...
 */
function injectTextRenderingRule(document) {
...
function stripJsCommentsParserSafe(source) {
    if (!source)
        return source;
    try {
...
function ensureExternalScriptTag(doc, src) {
    if (queryByAttr(doc, "src", src, "script"))
        return;
    const el = doc.createElement("script");
    el.setAttribute("src", src);
    doc.body.appendChild(el);
}
function hoistExternalScript(src, projectDir, doc, seenSrcs, chunks) {
    if (seenSrcs.has(src))
        return;
    seenSrcs.add(src);
...
function hoistCompositionScripts(container, opts) {
    for (const scriptEl of [...container.querySelectorAll("script")]) {
        const externalSrc = (scriptEl.getAttribute("src") || "").trim();
        if (externalSrc) {
            hoistExternalScript(externalSrc, opts.projectDir, opts.document, opts.seenCompScriptSrcs, opts.compScriptChunks);
        }
        else {
            opts.compScriptChunks.push(opts.compId
                ? wrapScopedCompositionScript(scriptEl.textContent || "", opts.compId, "[HyperFrames] composition script error:", opts.runtimeScope, opts.runtimeCompId || opts.compId, opts.authoredRootId)
                : wrapInlineScriptWithErrorBoundary(scriptEl.textContent || "", "[HyperFrames] composition script error:"));
        }
        scriptEl.remove();
    }
}
export async function bundleToSingleHtml(projectDir, options) {
    ...
    const subCompResult = inlineSubCompositions(document, subCompositionHosts, {
        ...
        onMissingComposition: (srcPath, reason) => {
            console.warn(`[Bundler] Skipping sub-composition "${srcPath}": ${reason ?? "the file could not be found"}.`);
        },
    });
    ...
    for (const scriptItem of subCompResult.scriptItems) {
        ...
        if (seenCompScriptSrcs.has(extSrc))
            continue;
    ...
    const candidateInlineHosts = trackedCompositionHosts.filter((host) => !host.hasAttribute("data-composition-src"));
    for (const templateEl of [...document.querySelectorAll("template[id]")]) {
        ...
        if (!compId)
            continue;
        const hosts = candidateInlineHosts.filter((host) => hostIdentityByElement.get(host)?.authoredCompositionId === compId &&
        ...
        for (const host of hosts) {
            ...
            pushSubCompVariableStyles(innerDoc, innerRoot, mergedVariables, runtimeScope, compStyleChunks);
            if (innerRoot) {
                // Hoist styles into the collected style chunks
                for (const styleEl of [...innerRoot.querySelectorAll("style")]) {
                    const css = styleEl.textContent || "";
                    compStyleChunks.push(compId
    ...
    for (const link of compExternalLinks) {
        ...
        if (!document.querySelector(`link[href="${escapedHref}"]`)) {
            ...
            if (link.crossorigin != null)
                linkEl.setAttribute("crossorigin", link.crossorigin);
    ...
    for (const el of [...document.querySelectorAll("[src], [href], [poster], [xlink\\:href]")]) {
        for (const attr of ["src", "href", "poster", "xlink:href"]) {
            ...
            if (isExternalSvgFragmentUse(el, attr, value))
                continue;
    ...
    if (options?.inlineColorGradingLuts !== false) {
        for (const el of [...document.querySelectorAll(`[${HF_COLOR_GRADING_ATTR}]`)]) {
            const value = el.getAttribute(HF_COLOR_GRADING_ATTR);
            if (value) {
                el.setAttribute(HF_COLOR_GRADING_ATTR, rewriteColorGradingLutWithInlinedAssets(value, projectDir));
            }
        }
    }
 ...
 */
export function emitRootCompositionVariableStyles(document, variablesByComp = {}, overrides = {}) {
...
function authoredDefinesPredicate(document) {
 ...
 */
function makeVariableLayer(authoredDefines, overrides) {
    return (declared, hostValues) => {
        const out = {};
        for (const [id, value] of Object.entries(declared)) {
            if (!authoredDefines(id))
                out[id] = value;
        }
        ...
        for (const [id, value] of Object.entries(overrides)) {
            if (id in declared || id in hostValues)
                out[id] = value;
        }
        return out;
    };
 ...
 */
function hostScopedVariableRules(variablesByComp, overrides, authoredDefines) {
    const rules = [];
    for (const [compId, vars] of Object.entries(variablesByComp)) {
        const withOverrides = {};
        for (const [id, value] of Object.entries(vars)) {
            if (!authoredDefines(id))
                withOverrides[id] = value;
        }
        for (const [id, value] of Object.entries(overrides)) {
            if (id in vars)
                withOverrides[id] = value;
        }
        const rule = compositionVariablesCssBlock(withOverrides, cssAttributeSelector("data-composition-id", compId));
        if (rule)
            rules.push(rule);
    }
    return rules;
}
function rootDeclaredVariableRules(document, layerFor) {
 ...
 */
function declarerVariableRules(document, layerFor) {
    const rules = [];
    let markerSeq = 0;
    for (const el of [...document.querySelectorAll("[data-composition-variables]")]) {
        const declared = readDeclaredDefaults(el);
        const hostEl = typeof el.closest === "function" ? el.parentElement?.closest("[data-variable-values]") : null;
 ...
 */
function pushSubCompVariableStyles(innerDoc, innerRoot, mergedVariables, runtimeScope, compStyleChunks) {
    if (!runtimeScope)
        return;
    ...
    if (innerRootForVars)
        Object.assign(declaredForCss, readDeclaredDefaults(innerRootForVars));
    const cssVars = compositionVariablesCssBlock({ ...declaredForCss, ...mergedVariables }, runtimeScope);
    if (cssVars)
        compStyleChunks.push(cssVars);
}
