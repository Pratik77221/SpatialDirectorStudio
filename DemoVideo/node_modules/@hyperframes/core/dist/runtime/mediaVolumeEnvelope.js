...
import { parseStrictFiniteTimingNumber } from "./playbackRate.js";
/**
...
function recordVolumeSample(keyframes, previousSample, sample, isFinalSample) {
...
function resolveVolumeProbeWindow(el, compositionDuration) {
 ...
 * each read.
 */
export function probeAndCacheElementVolume(mediaEl, timeline, compositionDuration, cache, options = {}) {
    if (options.allowLiveTimelineSeek === false)
        return;
    if (!timeline)
        return;
    ...
    const originalTime = typeof timeline.totalTime === "function"
