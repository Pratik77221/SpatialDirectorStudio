 ...
 */
import { cssVariableName, detectSlugCollisions } from "../tokenSlug.js";
export function getVariables() {
    ...
    for (const el of declarers)
        warnUnknownEnumValues(el, merged);
    return merged;
}
/**
 ...
 * (nothing fell back).
 */
const warnedUnknownEnumValues = new Set();
/**
 ...
 */
function compositionLabel(declarer, compositionId) {
    return (compositionId?.trim() ||
...
function declaredOptions(entry) {
 ...
 */
function unknownEnumValue(entry, resolved) {
    if (typeof entry.id !== "string")
        return null;
    const value = resolved[entry.id];
    if (value === undefined || value === null)
        return null;
    if ("default" in entry && String(value) === String(entry.default))
        return null;
    ...
    if (allowed.length === 0 || allowed.some((v) => String(v) === String(value)))
        return null;
    return { id: entry.id, value, allowed };
}
export function warnUnknownEnumValues(declarer, resolved, compositionId) {
    if (!declarer)
        return;
    ...
    for (const entry of entries) {
        ...
        if (warnedUnknownEnumValues.has(key))
            continue;
        warnedUnknownEnumValues.add(key);
        const fallback = "default" in entry ? JSON.stringify(entry.default) : "the composition default";
        console.warn(`[hyperframes] runtime_unknown_enum_value: ${move} variable "${found.id}" got ${JSON.stringify(found.value)}, which is not a declared option (${found.allowed.join(", ")}). Rendering ${fallback} instead.`);
    }
}
/** Test-only: clear the per-page dedupe set. */
export function resetUnknownEnumWarnings() {
...
export function readDeclaredDefaults(root) {
    ...
    for (const entry of readDeclarations(root)) {
        if (typeof entry.id !== "string" || !("default" in entry))
            continue;
...
function hasInlineStyle(target) {
    return "style" in target && typeof target.style?.setProperty === "function";
}
/**
 ...
 */
export function applyCssVariables(target, variables) {
    ...
    if (applied.length > 0)
        target.setAttribute(APPLIED_VARS_ATTR, applied.join(" "));
}
/** Keep only variables whose CSS custom property is not already defined. */
export function filterVariablesIfAbsent(target, variables, view) {
    const filtered = {};
    for (const [id, value] of Object.entries(variables)) {
        const name = cssVariableName(id);
        const existing = (hasInlineStyle(target) ? target.style.getPropertyValue(name) : "") ||
...
export function clearAppliedCssVariables(target) {
    if (!hasInlineStyle(target))
        return;
 ...
 */
export function injectCompositionCssVariables(doc) {
    const declarers = new Set();
    ...
    for (const group of detectSlugCollisions(allIds)) {
        console.warn(`composition variables ${group.join(", ")} collapse to the same CSS property ${cssVariableName(group[0] ?? "")} — rename one to avoid cross-talk`);
    }
}
...
function applyDeclaredForElement(el, overrides, view) {
    ...
    for (const [id, value] of Object.entries(declared)) {
        if (id in overrides)
            continue; // overrides applied below, always win
        const name = cssVariableName(id);
        // define-if-absent: respect authored/compiled definitions
        const existing = el.style.getPropertyValue(name) ||
...
export function parseHostVariableValues(host) {
...
export function readRenderOverrides() {
