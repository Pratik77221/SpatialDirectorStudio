...
import { inspectRuntimeProtocol, runtimeProtocolFpsToNumber, runtimeProtocolMetadata, } from "./protocol.js";
let runtimeProtocolFps = 30;
export function setRuntimeProtocolFps(fps) {
    runtimeProtocolFps = Number.isFinite(fps) && fps > 0 ? fps : 30;
}
export function postRuntimeMessage(payload) {
    try {
...
const CONTROL_HANDLERS = {
    ...
    "set-volume": (data, deps) => deps.onSetVolume(Math.max(0, Math.min(1, Number(data.volume ?? 1)))),
    "set-media-output-muted": (data, deps) => deps.onSetMediaOutputMuted(Boolean(data.muted)),
    "set-native-media-sync-disabled": (data, deps) => deps.onSetNativeMediaSyncDisabled(Boolean(data.disabled)),
    "set-web-audio-media-disabled": (data, deps) => deps.onSetWebAudioMediaDisabled(Boolean(data.disabled)),
    ...
    "set-color-grading": (data, deps) => deps.onSetColorGrading(data.target ?? null, data.grading ?? null),
    "set-color-grading-compare": (data, deps) => deps.onSetColorGradingCompare(data.target ?? null, data.compare ?? null),
    ...
    "clear-runtime-data": (data, deps) => {
        if (typeof data.channel === "string")
            deps.onClearRuntimeData?.(data.channel, data.requestId);
    },
};
function resolveSeekTimeSeconds(data, deps) {
...
function rejectUnsupportedProtocol(data) {
    ...
    postRuntimeMessage({
        ...
        details: {
            receivedVersion: typeof protocol.receivedVersion === "string" || typeof protocol.receivedVersion === "number"
...
function handleFlashElements(data) {
...
export function installRuntimeControlBridge(deps) {
    const handler = (event) => {
        const data = event.data;
        if (!data || data.source !== "hf-parent" || data.type !== "control")
            return;
        if (rejectUnsupportedProtocol(data))
            return;
 ...
 */
function flashElements(selectors, duration) {
