...
function createsPositionedStackingContext(style) {
    return style.position === "fixed" || style.position === "sticky";
}
function createsZIndexedStackingContext(element, style) {
    if (style.zIndex === "auto" || style.zIndex === "")
        return false;
    if (Z_INDEXED_POSITIONS.has(style.position))
        return true;
...
function createsVisualEffectStackingContext(style) {
    const opacity = Number.parseFloat(style.opacity);
    if (Number.isFinite(opacity) && opacity < 1)
        return true;
    if (style.getPropertyValue("isolation") === "isolate")
        return true;
...
function createsContainmentStackingContext(style) {
    const contain = style.getPropertyValue("contain").split(/\s+/);
    if (contain.some((value) => CONTAINMENT_VALUES.has(value)))
        return true;
    return STACKING_CONTAINER_TYPES.has(style.getPropertyValue("container-type"));
}
function createsWillChangeStackingContext(style) {
    return style
...
function createsCssStackingContext(element, style) {
    if (createsPositionedStackingContext(style))
        return true;
    if (createsZIndexedStackingContext(element, style))
        return true;
    if (createsVisualEffectStackingContext(style))
        return true;
    if (createsContainmentStackingContext(style))
        return true;
    return createsWillChangeStackingContext(style);
}
/** Stable within one DOM revision and unique even when authors duplicate ids. */
function cssStackingContextPath(element) {
    const indexes = [];
    let cursor = element;
 ...
 */
export function resolveCssStackingContextId(node) {
...
//# sourceMappingURL=stackingContext.js.map