 ...
 */
export function applyPositionEdits(doc, opts) {
    ...
    // is unavailable.
    const isStylable = (el) => RealmHTMLElement || RealmSVGElement
    ...
    const orphaned = doc.querySelectorAll(`[${EDIT_ORIGINAL_TRANSLATE_ATTR}]:not([${EDIT_BASE_X_ATTR}]):not([${EDIT_BASE_Y_ATTR}])`);
    for (let i = 0; i < orphaned.length; i++) {
        const el = orphaned[i];
        if (el === undefined || !isStylable(el))
            continue;
        const original = el.getAttribute(EDIT_ORIGINAL_TRANSLATE_ATTR) ?? "";
        if (original === "") {
...
const wrappedSeekFunctions = new WeakSet();
const observedSeekProperties = new WeakMap();
const observedGlobalProperties = new WeakMap();
/** Reapply SDK position edits after every render seek, including late-bound seeks. */
export function installPositionEditsSeekReapply(win) {
    const target = win;
    const reapply = () => {
        try {
    ...
    const isWrapped = (fn) => typeof fn === "function" &&
        (wrappedSeekFunctions.has(fn) ||
            Boolean(fn[SEEK_REAPPLY_WRAPPED]));
    ...
    const wrapFunction = (fn) => {
        if (typeof fn !== "function" || isWrapped(fn))
            return fn;
    ...
    const observeSeekProperty = (container, property) => {
        ...
        if (descriptor?.configurable === false) {
            ...
            if (typeof current === "function") {
                container[property] = wrapFunction(current);
                reapply();
            }
            return false;
        }
        let current = container[property];
    ...
    const observeGlobalContainer = (name, property) => {
        ...
        if (!globals?.has(name)) {
            ...
            Object.defineProperty(target, name, {
                ...
                set: (next) => {
                    value = next;
                    if (value)
                        observeSeekProperty(value, property);
                },
            });
            globals ??= new Set();
            globals.add(name);
