 ...
 * values.
 */
import { readVariablesForElement } from "./variableScope.js";
import { isScalarVariableValue as isScalar, isSafeMediaUrl, } from "@hyperframes/parsers/composition";
...
function valuesForElement(el, cache) {
 ...
 */
function setOwnTextPreservingChildren(el, text) {
 ...
 */
function findTopRoot(doc) {
    return (doc.querySelector("[data-hf-root]") ??
        doc.getElementById("stage") ??
        doc.body?.firstElementChild ??
        doc.body);
}
function applyCssCustomProperties(doc, cache) {
...
export function applyVariableBindings(doc) {
    const cache = new Map();
    ...
    for (const el of Array.from(doc.querySelectorAll("[data-var-src]"))) {
        ...
        if (!VAR_SRC_TAGS.has(el.tagName.toLowerCase())) {
            console.warn(`[hyperframes] Ignoring data-var-src on <${el.tagName.toLowerCase()}>: variable-bound src is only allowed on ${Array.from(VAR_SRC_TAGS).join("/")}.`);
            continue;
        }
    ...
    for (const el of Array.from(doc.querySelectorAll("[data-var-text]"))) {
        ...
        if (isScalar(value))
            setOwnTextPreservingChildren(el, String(value));
    }
}
//# sourceMappingURL=applyVariableBindings.js.map