 ...
 */
export function audioBandAt(hz) {
    if (!Number.isFinite(hz))
        return undefined;
    const first = BANDS[0];
    const last = BANDS.at(-1);
    if (first && hz < first.from)
        return first;
    if (last && hz >= last.to)
        return last;
...
const strength = (x, words) => x < 0.34 ? words[0] : x < 0.67 ? words[1] : words[2];
