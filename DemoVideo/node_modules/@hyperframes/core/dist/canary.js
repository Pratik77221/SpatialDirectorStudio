...
export function canaryBucket(feature, unitId) {
...
export function evaluateCanary(input) {
    ...
    if (input.override === false)
        return { enabled: false, reason: "forced_off" };
    const pct = Math.max(0, Math.min(100, Math.trunc(input.percentage)));
    if (pct <= 0)
        return { enabled: false, reason: "out_of_cohort" };
    ...
    if (pct >= 100)
        return { enabled: true, reason: "in_cohort" };
    if (input.exclude)
        return { enabled: false, reason: "excluded" };
    const unitId = input.unitId?.trim();
    if (!unitId)
        return { enabled: false, reason: "no_unit_id" };
 ...
 */
export function parseCanaryOverride(raw) {
...
export function canaryFeatureKey(name) {
 ...
 */
export function canaryReasonKey(name) {
 ...
 */
export function canaryFeatureProperties(entries) {
    const props = {};
