...
function resolveValue(start, vars) {
    const chain = [];
    const seen = new Set();
    let currentId = start;
...
function toEntryValue(resolvedType, raw) {
    if (resolvedType === "COLOR")
        return figmaColorToCss(raw);
...
export function tokensToVariables(vars, source) {
    const entries = [];
    const bindings = [];
    const sidecarTokens = [];
    for (const [figmaId, payload] of Object.entries(vars.variables)) {
        ...
        if (!entryType || value === null || !resolved)
            continue;
        entries.push({
...
//# sourceMappingURL=tokensToVariables.js.map