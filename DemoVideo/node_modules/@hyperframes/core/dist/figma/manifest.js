...
import { readJsonlValues } from "./jsonl.js";
 ...
 * scans the full file.
 */
export function isFigmaManifestRecord(value) {
    ...
    if (!("source" in provenance) || !("fileKey" in provenance) || !("nodeId" in provenance))
        return false;
    return (provenance.source === "figma" &&
...
export function findByFigmaNode(projectDir, fileKey, nodeId) {
...
export function findAllByFigmaNode(projectDir, fileKey, nodeId) {
    return readManifest(projectDir).filter((r) => r.provenance.source === "figma" &&
...
export function updateRecord(projectDir, record) {
