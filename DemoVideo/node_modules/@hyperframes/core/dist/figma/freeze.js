...
export function exceedsFreezeCap(byteLength) {
    return byteLength > MAX_FREEZE_BYTES;
}
export function freezeBytes(bytes, destPath) {
    if (bytes.length === 0)
 ...
 */
export function isAllowedFreezeUrl(url) {
    let parsed;
    try {
        parsed = new URL(url);
    }
...
export async function freezeUrl(url, destPath) {
    ...
    if (!res.ok)
        throw new Error(`freeze failed: HTTP ${res.status}`);
