...
const PROPERTY_MAP = { rotate: "rotation" };
...
function arrayAfterKey(src, key) {
...
function scalarAfterKey(src, key) {
    const m = new RegExp(`${escapeRegExp(key)}\\s*:\\s*("[^"]*"|[\\w.]+)`).exec(src);
    return m?.[1] ?? null;
}
function parseEase(transitionBlock) {
 ...
 */
function stripWrapTail(values, times, ease, duration) {
    let end = values.length;
    while (end > 2) {
        const tPrev = times[end - 2];
        const tCur = times[end - 1];
        if (tPrev === undefined || tCur === undefined)
            break;
        if ((tCur - tPrev) * duration >= WRAP_EPSILON_S)
            break;
        end -= 1;
    }
...
function extractTrackData(animate, transition, prop) {
    ...
    if (!timesSrc || !durationSrc || !ease)
        return null;
    const values = JSON.parse(valuesSrc);
    const times = JSON.parse(timesSrc);
    const duration = Number(durationSrc);
    if (values.length !== times.length || !Number.isFinite(duration))
        return null;
    return { values, times, ease, duration };
}
/** Parse one property's track out of the animate/transition blocks. */
function parsePropertyTrack(animate, transition, prop, repeat) {
    ...
    const segEase = raw.ease.length === segCount
        ? raw.ease
...
function parseNodeTracks(node, repeat) {
 ...
 */
export function motionContextToDocs(response, options) {
...
//# sourceMappingURL=motionContextToDocs.js.map