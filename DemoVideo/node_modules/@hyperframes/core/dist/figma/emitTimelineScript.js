...
export function emitTimelineScript(spec) {
    ...
    lines.push(`if (${missing}) { console.warn(${lit(`figma timeline ${spec.timelineId}: ${libs} not loaded — add the CDN <script> tags before this one`)}); return; }`);
    for (const ce of spec.customEases) {
        const [x1, y1, x2, y2] = ce.bezier;
        lines.push(`CustomEase.create(${lit(ce.name)}, "M0,0 C${num(x1)},${num(y1)} ${num(x2)},${num(y2)} 1,1");`);
    }
    lines.push("const tl = gsap.timeline({ paused: true });");
    for (const t of spec.tweens)
        lines.push(...emitTween(t));
    lines.push("window.__timelines = window.__timelines || {};");
    lines.push(`window.__timelines[${lit(spec.timelineId)}] = tl;`);
    lines.push("})();");
    return lines.join("\n");
}
//# sourceMappingURL=emitTimelineScript.js.map