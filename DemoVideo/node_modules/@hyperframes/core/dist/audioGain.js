...
export function clampAudioGain(value) {
    if (!Number.isFinite(value))
        return 1;
    return Math.max(0, Math.min(MAX_AUDIO_GAIN, value));
}
export function clampNativeMediaVolume(value) {
    if (!Number.isFinite(value))
        return 1;
 ...
 */
export function formatAudioGain(gain) {
 ...
 */
export function withUnclampedVolume(el, probe) {
    ...
    const descriptor = typeof HTMLMediaElement === "undefined"
        ? undefined
...
export function audioFaderPositionToGain(position) {
...
export function audioGainToFaderPosition(gain) {
...
export function audioGainToText(gain) {
    const safe = clampAudioGain(gain);
    if (safe === 0)
        return "-∞ dB";
    const db = 20 * Math.log10(safe);
...
//# sourceMappingURL=audioGain.js.map