...
import { COLOR_GRADING_ADJUST_KEYS, COLOR_GRADING_ADVANCED_LIMITS, COLOR_GRADING_COLOR_SPACE, COLOR_GRADING_CONTRACT_VERSION, COLOR_GRADING_CURVE_KEYS, COLOR_GRADING_DETAIL_KEYS, COLOR_GRADING_EFFECT_KEYS, COLOR_GRADING_HUE_CURVE_KEYS, COLOR_GRADING_LUT_KEYS, COLOR_GRADING_MAX_CURVE_POINTS, COLOR_GRADING_MAX_SECONDARIES, COLOR_GRADING_TOP_LEVEL_KEYS, COLOR_GRADING_WHEEL_KEYS, } from "@hyperframes/parsers/color-grading-contract";
export { compileHfColorCurve, compileHfHueCurve, HF_COLOR_CURVE_MAX_POINTS, } from "./colorGradingCurves.js";
...
export const HF_COLOR_GRADING_WHEEL_KEYS = COLOR_GRADING_WHEEL_KEYS;
export const HF_COLOR_GRADING_CURVE_KEYS = COLOR_GRADING_CURVE_KEYS;
export const HF_COLOR_GRADING_HUE_CURVE_KEYS = COLOR_GRADING_HUE_CURVE_KEYS;
...
export const HF_COLOR_GRADING_PRESETS = [
    ...
    preset("muted-editorial", "Muted Editorial", {
        exposure: -0.02,
        contrast: 0.08,
        highlights: -0.08,
        shadows: 0.06,
...
const RGB_CURVE_IDENTITY = [
    [0, 0],
    [1, 1],
];
const HUE_CURVE_IDENTITY = [];
const HUE_CURVE_LIMITS = {
...
const SECONDARY_HUE_DEFAULT = {
    center: 0,
    range: 180,
    softness: 0,
};
const SECONDARY_SOFT_RANGE_DEFAULT = {
    min: 0,
    max: 1,
    softness: 0.05,
};
...
const EFFECT_LIMIT_OVERRIDES = COLOR_GRADING_ADVANCED_LIMITS.effects;
...
function readLimitedValue(value, limit) {
    const parsed = typeof value === "number" ? value : Number(value);
    if (!Number.isFinite(parsed))
        return 0;
    return clamp(parsed, limit.min, limit.max);
}
function wrapDegrees(value, fallback = 0) {
    const parsed = typeof value === "number" ? value : Number(value);
    if (!Number.isFinite(parsed))
        return fallback;
...
function softRangeMask(value, min, max, softness) {
    if (value < min) {
 ...
 */
export function calculateHfColorGradingSecondaryMask(hue, saturation, luma, key) {
    const hueDistance = Math.abs(((((hue - key.hue.center + 540) % 360) + 360) % 360) - 180);
    const hueMask = key.hue.range < 179.999 && saturation < 0.001
        ? 0
...
function normalizeWheels(value) {
    const wheels = isRecord(value) ? value : {};
    return HF_COLOR_GRADING_WHEEL_KEYS.reduce((result, key) => {
...
function hasDuplicateCurveInputs(points) {
    return points.some((point, index) => index > 0 && point[0] === points[index - 1]?.[0]);
}
function completeCurveEndpoints(points) {
    if ((points[0]?.[0] ?? 0) > 0)
        points.unshift([0, 0]);
    if ((points.at(-1)?.[0] ?? 1) < 1)
        points.push([1, 1]);
}
function normalizeCurve(value) {
    if (!Array.isArray(value) || value.length < 2)
        return RGB_CURVE_IDENTITY;
    const points = [];
    ...
    if (hasDuplicateCurveInputs(points))
        return RGB_CURVE_IDENTITY;
    completeCurveEndpoints(points);
    if (points.length > COLOR_GRADING_MAX_CURVE_POINTS)
        return RGB_CURVE_IDENTITY;
    return points;
}
function normalizeCurves(value) {
    const curves = isRecord(value) ? value : {};
    return HF_COLOR_GRADING_CURVE_KEYS.reduce((result, key) => {
...
function normalizeHueCurvePoint(value, limit) {
    if (!Array.isArray(value) || value.length !== 2)
        return null;
...
function normalizeHueCurve(value, limit) {
...
function normalizeHueCurves(value) {
    const curves = isRecord(value) ? value : {};
    return HF_COLOR_GRADING_HUE_CURVE_KEYS.reduce((result, key) => {
...
function normalizeSoftRange(value) {
    ...
    return {
        ...
        softness: readLimitedValue(range.softness ?? SECONDARY_SOFT_RANGE_DEFAULT.softness, COLOR_GRADING_ADVANCED_LIMITS.secondarySoftRangeSoftness),
    };
}
function normalizeSecondaryHue(value) {
    const hue = isRecord(value) ? value : {};
    const range = readLimitedValue(hue.range ?? SECONDARY_HUE_DEFAULT.range, COLOR_GRADING_ADVANCED_LIMITS.secondaryHueRange);
    return {
...
function normalizeSecondaryCorrection(value) {
    return {
        hueShift: readLimitedValue(value.hueShift ?? 0, COLOR_GRADING_ADVANCED_LIMITS.secondaryHueShift),
...
function normalizeSecondary(value) {
    if (!isRecord(value) || !isRecord(value.key) || !isRecord(value.correction))
        return null;
    return {
...
function normalizeSecondaries(value) {
...
function readColorGradingObject(raw) {
...
export function resolveHfColorGradingVariables(raw, variables) {
    if (typeof raw === "string") {
        ...
        if (!trimmed.startsWith("{"))
            return raw;
        try {
            return resolveHfColorGradingVariables(JSON.parse(trimmed), variables);
        }
        catch {
            return raw;
        }
...
function getHfColorGradingPreset(id) {
    if (!id)
        return null;
    return PRESETS_BY_ID.get(id) ?? null;
}
export function normalizeHfColorGrading(raw) {
    const grading = readColorGradingObject(raw);
    if (!grading)
        return null;
    if (grading.enabled === false)
        return null;
    ...
    const adjust = HF_COLOR_GRADING_ADJUST_KEYS.reduce((result, key) => {
...
export function normalizeHfColorGradingWithVariables(raw, variables) {
    return normalizeHfColorGrading(resolveHfColorGradingVariables(raw, variables));
}
function hasWheelGrade(wheels) {
    return HF_COLOR_GRADING_WHEEL_KEYS.some((key) => Math.abs(wheels[key].amount) > 0.0001 || Math.abs(wheels[key].level) > 0.0001);
}
export function hasHfColorGradingRgbCurveValues(curves) {
    return HF_COLOR_GRADING_CURVE_KEYS.some((key) => curves[key].some(([input, output]) => Math.abs(input - output) > 0.0001));
}
export function hasHfColorGradingHueCurveValues(curves) {
    return HF_COLOR_GRADING_HUE_CURVE_KEYS.some((key) => curves[key].some(([, delta]) => Math.abs(delta) > 0.0001));
}
export function hasHfColorGradingSecondaryValues(secondaries) {
    return secondaries.some((secondary) => secondary.enabled &&
...
export function serializeHfColorGrading(grading) {
    ...
    if (!normalized)
        return "";
    const { enabled: _enabled, wheels, curves, hueCurves, secondaries, palette, ...serializable } = normalized;
...
export function hasHfColorGradingAuthoredValues(grading) {
    if (!grading)
        return false;
    if (grading.secondaries?.length)
        return true;
