...
async function shouldCopyAacSidecar(audioPath, options) {
    if (options?.audioCodec === "aac" || isAacSidecar(audioPath))
        return true;
    const audioExtension = extname(audioPath).toLowerCase();
    if (KNOWN_NON_AAC_AUDIO_EXTENSIONS.has(audioExtension))
        return false;
    try {
...
export function buildEncoderArgs(options, inputArgs, outputPath, gpuEncoder = null) {
    const { fps, codec = "h264", preset = "medium", quality = 23, bitrate, pixelFormat = "yuv420p", vp9CpuUsed, useGpu = false, } = options;
        ...
        if (shouldUseGpu) {
            ...
            if (lockGop) {
                if (typeof options.gopSize !== "number" ||
                    ...
                    options.gopSize <= 0) {
                    throw new Error(`[chunkEncoder] lockGopForChunkConcat=true requires a positive integer gopSize (received ${String(options.gopSize)})`);
                }
                gop = Math.floor(options.gopSize);
                args.push("-g", String(gop), "-keyint_min", String(gop), "-sc_threshold", "0", "-force_key_frames", `expr:eq(mod(n,${gop}),0)`);
            }
            ...
            if (preset === "ultrafast") {
                ...
                args.push(xParamsFlag, joinParams("aq-mode=3", "aq-strength=0.8", "deblock=1,1", colorParams, gopParams));
            }
        }
    ...
    if (codec === "h264" || codec === "h265") {
        ...
        if (gpuEncoder === "vaapi") {
            ...
            args.push("-vf", withEvenDimensionPad("scale=in_range=pc:out_range=tv", pixelFormat, options.width, options.height));
        }
...
export async function encodeFramesFromDir(framesDir, framePattern, outputPath, options, signal, config) {
    ...
    if (!result.success) {
        return {
            ...
            framesEncoded: 0,
            fileSize: 0,
            error: appendEncodeTimeoutMessage(formatFfmpegError(result.exitCode, result.stderr), result.terminationReason === "deadline", encodeTimeout),
...
export async function encodeFramesChunkedConcat(framesDir, framePattern, outputPath, options, chunkSizeFrames, signal, config) {
    ...
    for (let i = 0; i < chunkCount; i++) {
        ...
        const chunkResult = {
            ...
            error: processResult.success
                ? undefined
                : appendEncodeTimeoutMessage(`Chunk ${i} encode failed: ${processResult.stderr.slice(-400)}`, processResult.terminationReason === "deadline", encodeTimeout),
        };
    ...
    const concatResult = {
        ...
        error: concatProcessResult.success
            ? undefined
            : appendEncodeTimeoutMessage(`Chunk concat failed: ${concatProcessResult.stderr.slice(-400)}`, concatProcessResult.terminationReason === "deadline", encodeTimeout),
    };
...
export async function muxVideoWithAudio(videoPath, audioPath, outputPath, signal, config, fps) {
    const outputDir = dirname(outputPath);
    if (!existsSync(outputDir))
        mkdirSync(outputDir, { recursive: true });
...
export async function applyFaststart(inputPath, outputPath, signal, config, fps) {
