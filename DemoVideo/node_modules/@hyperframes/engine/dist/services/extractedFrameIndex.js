...
export function extractedFrameIndex(file, format) {
...
export function framePathsFromDirectory(outputDir, format) {
    const suffix = `.${format}`;
    const indexed = new Map();
    for (const file of readdirSync(outputDir)) {
        if (!file.startsWith(FRAME_FILENAME_PREFIX) || !file.endsWith(suffix))
            continue;
        const index = extractedFrameIndex(file, format);
        if (indexed.has(index)) {
            throw new ExtractedFrameSequenceError(`Duplicate extracted frame index ${index}: ${indexed.get(index)} and ${file}`);
        }
    ...
    for (let index = 0; index < indexed.size; index += 1) {
        ...
        if (!path) {
            throw new ExtractedFrameSequenceError(`Missing extracted frame index ${index} in ${outputDir}`);
        }
        ordered.set(index, path);
    }
    return ordered;
}
//# sourceMappingURL=extractedFrameIndex.js.map