 ...
 */
export function getDrawElementVerificationDetails(err) {
    let e = err;
    for (let depth = 0; depth < 5 && typeof e === "object" && e !== null; depth++) {
        const rec = e;
        if (rec.deVerificationFailure === true) {
            ...
            if (typeof rec.frameIndex === "number")
                details.frameIndex = rec.frameIndex;
            if (typeof rec.failedDb === "number")
                details.failedDb = rec.failedDb;
...
export async function decodeDynamicCssBackgroundImages(page) {
    await page.evaluate(async () => {
        const root = globalThis;
        const decode = (root.__hfDecodeDynamicCssBackgroundImages ??= async () => {
            const decoded = (root.__hf_css_background_decoded ??= new Set());
            const urls = [];
...
function appendBrowserDiagnostic(session, text) {
    ...
    try {
        tweenCount = await page.evaluate(() => {
            ...
            for (const timeline of Object.values(timelines)) {
                ...
                if (typeof maybeTimeline?.getChildren !== "function")
                    continue;
...
async function recordSessionInitTelemetry(session, initStart) {
    ...
    appendBrowserDiagnostic(session, `[FrameCapture:INIT] complete initDurationMs=${telemetry.initDurationMs} tweenCount=${telemetry.tweenCount}` +
...
export function formatNavigationFailureDiagnostic(input) {
    ...
    return (`[FrameCapture:ERROR] page.goto failed ` +
...
export function formatNavigationStartDiagnostic(input) {
    return (`[FrameCapture:NAV] page.goto start ` +
 ...
 */
export function warmupFrameTimeTicks(state, intervalMs) {
 ...
 */
export function deriveBeginFrameTimeTicks(state, warmupIntervalMs, captureIntervalMs) {
    const legacyCaptureTimeTicks = (state.ticks + BEGIN_FRAME_CAPTURE_HEADROOM_INTERVALS) * captureIntervalMs;
    const legacyCommitTimeTicks = deriveBeginFrameCommitTimeTicks(legacyCaptureTimeTicks, captureIntervalMs);
    const lastWarmupTimeTicks = Math.max(0, state.ticks - 1) * warmupIntervalMs;
    if (legacyCommitTimeTicks > lastWarmupTimeTicks)
        return legacyCaptureTimeTicks;
    const monotonicCaptureTimeTicks = warmupFrameTimeTicks(state, warmupIntervalMs) +
...
export function deriveBeginFrameProbeTimeTicks(captureTimeTicks, captureIntervalMs) {
    return Math.max(0, captureTimeTicks - BEGIN_FRAME_PROBE_LEAD_INTERVALS * captureIntervalMs);
}
export function deriveBeginFrameTimelineTicks(state, warmupIntervalMs, captureIntervalMs) {
...
export function prepareBeginFrameTimeline(session, state, warmupIntervalMs) {
    const timeline = deriveBeginFrameTimelineTicks(state, warmupIntervalMs, session.beginFrameIntervalMs);
...
export async function driveWarmupTicks(options, state) {
    ...
    while (true) {
        if (options.lockWarmupTicks) {
            ...
            if (state.ticks >= LOCKED_WARMUP_TICKS)
                return;
        }
        else {
...
export function resolveCaptureSessionOptions(options, browserVersion, platform = process.platform) {
    return {
        ...options,
        captureBeyondViewport: options.captureBeyondViewport ?? shouldDefaultCaptureBeyondViewport(browserVersion, platform),
    };
}
...
async function initDrawElementOrTransparentBackground(session, page, logInitPhase) {
    ...
    if (useDrawElement) {
        ...
        if (!forceDE && !(await isPsnrFilterAvailable())) {
            ...
            console.warn(`[engine] fast capture: falling back to ${session.launchCaptureMode} capture — ` +
...
export async function createCaptureSession(serverUrl, outputDir, options, onBeforeCapture = null, config) {
    ...
    const effectiveForceScreenshot = applyConcreteGpuScreenshotClamp(forceScreenshot, resolvedGpuMode, config);
    const preMode = headlessShell &&
        isLinux &&
...
async function constructCaptureSession(input) {
    ...
    if (process.env.HF_FAST_CAPTURE_AUTOALPHA !== undefined) {
        console.warn("[engine] HF_FAST_CAPTURE_AUTOALPHA is retired and ignored — the requestPaint " +
...
export function formatConsoleDiagnostic(type, text, locationUrl) {
    const isFontLoadError = isFontResourceError(type, text, locationUrl);
    if (isFontLoadError)
        return { text: `[Browser] ${text}`, suppressHostLog: true };
...
function buildZeroDurationDiagnostic(diag) {
    ...
    if (!diag.hasTimeline) {
        hints.push("No GSAP timeline registered (window.__timelines is empty). " +
    ...
    if (diag.declaredDuration <= 0 && !diag.hasTimeline) {
        hints.push('Fix: add data-duration="<seconds>" to your root <div data-composition-id="..."> element.');
    }
...
async function pollHfReady(page, timeoutMs, intervalMs = 100) {
    ...
    throw new Error(`[FrameCapture] window.__hf not ready after ${timeoutMs}ms. ` +
...
async function pollVideosReady(page, skipIds, timeoutMs, intervalMs = 100) {
    const check = async () => {
        return Boolean(await page.evaluate((skipIdList) => {
            ...
            return (vids.length === 0 ||
                vids.every((v) => {
                    ...
                    if (ve.error)
                        return true;
                    if (ve.networkState === HTMLMediaElement.NETWORK_NO_SOURCE)
                        return true;
                    return false;
                }));
        }, skipIds));
...
export async function pollImagesReady(page, timeoutMs, intervalMs = 100) {
    const check = async () => {
        return Boolean(await page.evaluate(() => {
            ...
            return (imgs.length === 0 ||
                imgs.every((img) => {
                    ...
                    if (ie.complete && ie.naturalWidth === 0)
                        return true;
                    if (ie.complete && ie.naturalWidth > 0)
                        return true;
                    return false;
...
export async function collectMediaReadinessWarnings(page, skipIds, timeoutMs) {
    const snapshot = await page.evaluate((skipIdList) => {
        ...
        for (const img of document.querySelectorAll("img")) {
            ...
            if (!src || src.startsWith("data:"))
                continue;
            if (!img.complete)
                result.pendingImages.push(img.src || src);
            else if (img.naturalWidth <= 0)
                result.failedImages.push(img.src || src);
        }
        ...
        for (const media of document.querySelectorAll("video")) {
            ...
            if (skipped.has(mediaElement.id))
                continue;
            const src = mediaElement.currentSrc || mediaElement.getAttribute("src") || "(no src)";
            const failed = Boolean(mediaElement.error) ||
                mediaElement.networkState === HTMLMediaElement.NETWORK_NO_SOURCE;
            const pending = !failed && mediaElement.readyState < HTMLMediaElement.HAVE_CURRENT_DATA;
            if (failed)
                result.failedVideos.push(src);
            else if (pending)
                result.pendingVideos.push(src);
        }
        return result;
    }, skipIds);
    const warnings = [];
    const append = (code, mediaType, sources) => {
        if (sources.length === 0)
            return;
...
function recordCaptureWarnings(session, warnings) {
    for (const warning of warnings) {
        const key = `${warning.code}:${JSON.stringify(warning.details ?? {})}`;
        if (session.warnings.some((existing) => `${existing.code}:${JSON.stringify(existing.details ?? {})}` === key)) {
            continue;
        }
...
function recordSubTimelineWarning(session, timeoutMs) {
 ...
 */
export function buildLiveMapWarning(libraries) {
    return {
        code: "live_map_detected",
        message: `Live map viewport(s) detected in the composition (${libraries.join(", ")}). ` +
...
async function recordLiveMapWarning(session, page) {
    const libraries = (await page.evaluate((markers) => markers.filter((m) => document.querySelector(m.selector) !== null).map((m) => m.library), LIVE_MAP_MARKERS));
    ...
    await page.evaluate((metadataHints) => {
        for (const hint of metadataHints) {
            ...
            const video = (window.__hfMediaEl?.(hint.id) ??
                document.getElementById(hint.id));
            if (!video)
                continue;
...
export function classifyConsoleScriptFailure(type, text) {
    if (type !== "error")
        return null;
    ...
    if (/failed to find a valid digest in the ['"]integrity['"] attribute/i.test(text) &&
...
export async function initializeSession(session) {
    ...
    const pageNavigationTimeout = session.config?.pageNavigationTimeout ?? DEFAULT_CONFIG.pageNavigationTimeout;
    ...
    const gotoEntryPage = async () => {
        appendBrowserDiagnostic(session, formatNavigationStartDiagnostic({
        ...
        try {
            ...
            appendBrowserDiagnostic(session, formatNavigationFailureDiagnostic({
                captureMode: session.captureMode,
                url,
                timeoutMs: pageNavigationTimeout,
        ...
        const [videosReady] = await Promise.all([
            ...
            pollImagesReady(page, pageReadyTimeout).then(async (ready) => {
                if (!ready) {
                    ...
                    console.warn(`[FrameCapture] Some image elements did not load within ${pageReadyTimeout}ms: ${failedImages}. ` +
        ...
        if (!videosReady) {
            const failedVideos = await page.evaluate((skipIdList) => {
                ...
                return Array.from(document.querySelectorAll("video"))
                    ...
                    .filter((v) => v.readyState < 2 && !v.error)
                    .map((v) => v.src || v.getAttribute("src") || "(no src)")
                    .join(", ");
            }, skipVideoIds);
    ...
    recordCaptureWarnings(session, await collectMediaReadinessWarnings(page, bfSkipVideoIds, pageReadyTimeout));
    await recordLiveMapWarning(session, page);
    await recordSessionInitTelemetry(session, initStart);
    ...
    // LOCKED_WARMUP_TICKS.
    const preparedBeginFrameTimeline = prepareBeginFrameTimeline(session, warmupState, warmupIntervalMs);
    // drawElement or transparent-background init — runs after page is fully ready.
 ...
 */
export async function waitForPendingSeekCompletion(page) {
    await page.evaluate(async () => {
        const waitForCompletion = window.__hfWaitForSeekCompletion;
...
async function prepareFrameForCapture(session, frameIndex, time) {
    ...
    await page.evaluate(async () => {
        const runtime = window.__hf?.colorGrading;
...
export function isStaticDedupFrameAnalysisSafe(totalFrames) {
    return (Number.isFinite(totalFrames) &&
...
export async function computeStaticFrameSet(page, fps) {
    ...
    if (hasTimelineCall)
        reasons.push("tl.call() side effect (not seek-safe)");
 ...
 *   `captureFrameCore`.
 */
export async function discardWarmupCapture(session, frameIndex = 0, time = 0, innerCapture = captureFrameCore) {
 ...
 */
export function computeDeVerifySampleFractions(k) {
    if (k <= 0)
        return [];
    if (k === 1)
        return [0.95];
