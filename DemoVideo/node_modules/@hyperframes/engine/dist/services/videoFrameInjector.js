...
import { HF_COLOR_GRADING_CANVAS_ID_PREFIX, RENDER_FRAME_ID_PREFIX, RENDER_FRAME_ID_SUFFIX, renderFrameIdForRenderId, } from "@hyperframes/core";
/**
...
export function createVideoFrameInjector(frameLookup, config) {
    ...
    return async (page, time) => {
        ...
        if (updates.length > 0) {
            ...
            // frame blank.
            const injectedIds = new Set(await injectVideoFramesBatch(page, updates.map((u) => ({ videoId: u.videoId, dataUri: u.dataUri }))));
...
async function setVideoElementsVisibility(page, videoIds, visible) {
    if (videoIds.length === 0)
        return;
    await page.evaluate((entries, canvasIdPrefix, shouldShow) => {
        const apply = (node) => {
    ...
    videoIds.map((id) => ({ id, frameId: renderFrameIdForRenderId(id) })), HF_COLOR_GRADING_CANVAS_ID_PREFIX, visible);
}
/**
...
export async function queryVideoElementBounds(page, videoIds) {
    ...
    return page.evaluate((ids) => {
        const resolveVideo = (id) => (window.__hfMediaEl?.(id) ?? document.getElementById(id));
        return ids.map((id) => {
...
export async function queryElementStacking(page, nativeHdrIds) {
    ...
    (hdrIdList, prefix, suffix) => {
        ...
        function getViewportMatrix(node) {
            ...
            for (let i = chain.length - 1; i >= 0; i--) {
                ...
                if (hasIndividual || hasTransform) {
                    ...
                    if (hasIndividual)
                        mat = mat.multiply(individualTransform);
                    if (hasTransform) {
                        try {
        ...
        return results;
    }, hdrIds, RENDER_FRAME_ID_PREFIX, RENDER_FRAME_ID_SUFFIX);
}
