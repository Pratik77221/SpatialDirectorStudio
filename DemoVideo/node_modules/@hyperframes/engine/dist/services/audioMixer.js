...
import { downloadToTemp, isHttpUrl, UrlDownloadError, writeUrlDownloadTelemetry, } from "../utils/urlDownloader.js";
...
import { HF_AUDIO_AUTOMATION_ATTR, parseAutomation, resolveAutomation, sampleAutomationLane, VOLUME_TARGET, } from "@hyperframes/core/audio-automation";
import { chainTailSeconds } from "@hyperframes/core/audio-fx-tail";
import { MEDIA_RENDER_ID_ATTR, normalizePlaybackRate, parseStrictFiniteTimingNumber, readMediaStart, } from "@hyperframes/core";
 ...
 */
function safePathSegment(id, fallbackIndex) {
...
function preparedAudioOutputArgs(srcPath, playbackRate) {
    return stereoOutputArgs(srcPath).then((channelArgs) => {
        ...
        if (atempo)
            filters.push(atempo);
        if (filters.length > 0)
            outputArgs.push("-af", filters.join(","));
        return outputArgs;
    });
}
...
function legacyFilterScriptOptionIsUnsupported(stderr) {
    return (/filter_complex_script/i.test(stderr) &&
...
async function stereoOutputArgs(srcPath) {
    try {
        const { channels } = await extractAudioMetadata(srcPath);
        if (channels === 1)
            return ["-af", STEREO_CHANNEL_FILTER];
    }
    catch {
...
function buildVolumeExpression(track, ignoreKeyframes = false) {
    ...
    if (keyframes.length === 0)
        return `volume=${formatFilterNumber(staticVolume)}`;
...
function boundedDetail(message, maxLength = 2_000) {
    const redacted = message
        .replace(/\bhttps?:\/\/[^\s"'<>]+/gi, "<redacted-url>")
        .replace(/\bfile:\/\/[^\s"'<>]+/gi, "<redacted-path>")
        .replace(/\b[A-Za-z]:[\\/].+?(?=:\s[A-Z]|\s(?:ENOENT|EACCES|EPERM)\b|\r?$)/gm, "<redacted-path>")
        .replace(/(^|[\s"'(])\/.+?(?=:\s[A-Z]|\s(?:ENOENT|EACCES|EPERM)\b|\r?$)/gm, "$1<redacted-path>");
    return redacted.length <= maxLength ? redacted : `${redacted.slice(0, maxLength - 1)}…`;
}
function probeFailure(message, elementId) {
    ...
    const invalidMedia = /(?:invalid data found|could not find codec parameters|moov atom not found|no audio stream)/i.test(message);
    return {
        stage: "probe",
        reason: cancelled
            ? "cancelled"
...
function downloadFailure(error, elementId) {
    ...
    const invalidSource = error instanceof UrlDownloadError
        ? error.kind === "http_not_found" ||
            error.kind === "http_rejected" ||
            error.kind === "invalid_payload" ||
            error.kind === "cancelled"
        : /(?:invalid URL|only HTTPS|private\/reserved|HTTP (?:400|401|403|404|405|410|422)\b)/i.test(message);
...
function ffmpegFailure(stage, result, elementId) {
    const stderr = result.stderr ?? "";
    let reason = "ffmpeg_failed";
    let owner = "system";
    let retryable = false;
    ...
    else if (/(?:unrecognized option|option (?:was )?not found|no option name near)/i.test(stderr)) {
        reason = "ffmpeg_unsupported";
    }
    else if ((stage === "extract" || stage === "prepare") &&
    ...
    return {
        ...
        retryable,
        elementId,
        detail: boundedDetail(result.error?.message
 ...
 */
export function volumeLaneKeyframes(automation, trackStart, duration) {
...
export function parseAudioElements(html) {
    ...
    const resolveStart = (el) => el.getAttribute("data-start") ? resolveReferencedStart(document, el, startCache, visiting) : 0;
    ...
    const isHidden = (el) => {
        for (let current = el; current; current = current.parentElement) {
            if (current.hasAttribute("data-hidden"))
                return true;
        }
        return false;
    };
    ...
    const groupsById = new Map(resolveAudioGroups(document).map((group) => [group.id, group]));
    ...
    const build = (el, id, src, type) => {
        ...
        return {
            ...
            playbackRate: normalizePlaybackRate(playbackRateAttr ? parseFloat(playbackRateAttr) : Number.NaN),
    ...
    for (const el of document.querySelectorAll("audio[id]")) {
        ...
        if (!id || !src || isHidden(el) || memberGroupHidden(el))
            continue;
        if (isKnownInactiveTimelineWindow(el, resolveStart(el)))
            continue;
...
async function extractAudioFromVideo(videoPath, outputPath, options, signal, config) {
    ...
    if (options?.duration !== undefined)
        args.push("-t", String(options.duration * playbackRate));
...
async function mixAudioTracks(tracks, outputPath, totalDuration, signal, config) {
    ...
    const buildFilterComplex = (ignoreAutomation) => {
        ...
        tracks.forEach((track, i) => {
            ...
            // middle two.
            filterParts.push(`[${i}:a]atrim=0:${formatFilterNumber(trimDuration)},${volumeFilter},adelay=${delayMs}|${delayMs},apad,asetpts=N/SR/TB,atrim=0:${formatFilterNumber(totalDuration)}[a${i}]`);
        });
...
function groupNormalizeOptionUnsupported(stderr) {
    return (/normalize/i.test(stderr) &&
...
async function mixGroupMembers(memberTracks, outputPath, totalDuration, signal, config) {
    ...
    const runOnce = async (useNormalize, ignoreKeyframes = false) => {
    ...
    if (signal?.aborted)
        return { success: false, error: "Group sub-mix cancelled" };
    if (!result.success)
...
export async function processCompositionAudio(elements, baseDir, workDir, outputPath, totalDuration, signal, config, compiledDir) {
    ...
    await Promise.all(elements.map(async (element) => {
        ...
        try {
            ...
            if (isHttpUrl(srcPath)) {
                try {
                    srcPath = await downloadToTemp(srcPath, workDir, undefined, effectiveSignal, undefined, {
            ...
            if (await isNotMediaPayload(srcPath)) {
                failures.push({
                    ...
                    retryable: false,
                    elementId: element.id,
                    detail: boundedDetail(`Audio element ${element.id} source is a text document (HTML/XML/JSON), not media`),
                });
                return;
            }
            ...
            if (element.end - element.start <= 0) {
                ...
                try {
                    ...
                    failures.push(probeFailure(err instanceof Error ? err.message : String(err), element.id));
                    return;
                }
            ...
            if (element.type === "video") {
                ...
                }, effectiveSignal, config);
                if (!extractResult.success) {
                    failures.push(extractResult.failure
                ...
                const prepResult = await prepareAudioTrack(srcPath, trimmedPath, element.mediaStart, element.end - element.start, element.playbackRate, effectiveSignal, config);
                if (!prepResult.success) {
                    failures.push(prepResult.failure
            ...
            const automation = element.automation
                ? resolveAutomation(parseAutomation(element.automation), element.fxChain ? parseAudioFxChain(element.fxChain) : undefined)
                : null;
            ...
            const envelope = envelopeKeyframes && envelopeKeyframes.length > 0
                ? {
            ...
            if (element.fxChain) {
                ...
                const fxResult = await applyAudioFxChain(audioSrcPath, chain, join(workDir, `${element.id}-fx.wav`), {
                    trackId: element.id,
            ...
            if (envelope && !bakedEnvelope) {
                bakedEnvelope = applyVolumeEnvelopeToWav(audioSrcPath, envelope.keyframes, envelope.trackStart, envelope.baseVolume);
            }
            const track = {
                id: element.id,
            ...
            if (element.groupId) {
                ...
                if (bucket)
                    ...
                    groupTracks.set(element.groupId, [track]);
            ...
            failures.push({
                ...
                retryable: false,
                elementId: element.id,
                detail: boundedDetail(`Audio processing failed for element ${element.id}: ${err instanceof Error ? err.message : String(err)}`),
            });
        }
    })).catch((err) => {
    ...
    const bail = () => {
        ...
        return {
            ...
            tracksProcessed: tracks.length,
            error: boundedDetail(`Audio processing failed: ${failures.map((failure) => failure.detail).join(", ")}`),
            failures,
        };
    };
    ...
    const groupsDegradedAutomation = [];
    let groupIndex = -1;
    for (const [groupId, memberTracks] of groupTracks) {
        ...
        try {
            const subMix = await mixGroupMembers(memberTracks, groupWavPath, totalDuration, effectiveSignal, config);
            if (!subMix.success) {
                failures.push({
                    stage: "mix",
                    reason: "ffmpeg_failed",
                    ...
                    retryable: false,
                    elementId: groupId,
                    detail: boundedDetail(`Group sub-mix failed for group ${groupId}: ${subMix.error ?? "unknown"}`),
                });
                continue;
            }
            if (subMix.degradedAutomation)
                groupsDegradedAutomation.push(groupId);
            ...
            const automation = meta.automation
                ? resolveAutomation(parseAutomation(meta.automation), meta.fxChain ? parseAudioFxChain(meta.fxChain) : undefined)
                : null;
            const laneKeyframes = automation ? volumeLaneKeyframes(automation, 0, totalDuration) : null;
            const envelope = laneKeyframes && laneKeyframes.length > 0
            ...
            if (meta.fxChain) {
                ...
                const fxResult = await applyAudioFxChain(groupSrcPath, chain, join(workDir, `group-${pathId}-fx.wav`), {
                    trackId: groupId,
                    signal: effectiveSignal,
                    ...(automation ? { automation } : {}),
                    ...(envelope ? { envelope } : {}),
            ...
            if (envelope && !bakedEnvelope) {
                bakedEnvelope = applyVolumeEnvelopeToWav(groupSrcPath, envelope.keyframes, envelope.trackStart, envelope.baseVolume);
            }
            ...
            failures.push({
                ...
                retryable: false,
                elementId: groupId,
                detail: boundedDetail(`Audio processing failed for group ${groupId}: ${err instanceof Error ? err.message : String(err)}`),
            });
        }
    }
    if (failures.length > 0)
    ...
    const degradedNote = groupsDegradedAutomation.length > 0
