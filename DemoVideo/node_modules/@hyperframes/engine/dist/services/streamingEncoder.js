...
import { ManagedChildProcess, } from "../utils/managedChildProcess.js";
...
export function createFrameReorderBuffer(startFrame, endFrame) {
    ...
    const enqueueAt = (frame, resolve, reject) => {
...
export function buildStreamingArgs(options, outputPath, gpuEncoder = null) {
    const { fps, codec = "h264", preset = "medium", quality = 23, bitrate, pixelFormat = "yuv420p", vp9CpuUsed, useGpu = false, imageFormat = "jpeg", } = options;
    ...
    if (options.rawInputFormat) {
        ...
        args.push("-f", "rawvideo", "-pix_fmt", options.rawInputFormat, "-s", `${options.width}x${options.height}`, "-framerate", fpsToFfmpegArg(fps));
        if (inputColorTrc) {
        ...
        args.push("-f", "image2pipe", "-vcodec", inputCodec, "-framerate", fpsToFfmpegArg(fps), "-i", "-");
    }
...
export async function spawnStreamingEncoder(outputPath, options, signal, config) {
    ...
    const waitForDrainOrExit = async (stdin) => {
        ...
        try {
            const drainPromise = once(stdin, "drain", { signal: abortController.signal }).then(() => "drain");
            const closePromise = once(ffmpeg, "close", { signal: abortController.signal }).then(() => "exit");
            const racePromise = Promise.race([drainPromise, closePromise]).catch((err) => {
    ...
    const encoder = {
        ...
        close: async () => {
            ...
            if (exitCode !== 0) {
                const inactivitySuffix = terminationReason === "inactivity"
