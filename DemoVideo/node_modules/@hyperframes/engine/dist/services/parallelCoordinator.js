...
import { createCaptureSession, initializeSession, closeCaptureSession, captureFrame, captureFrameToBufferPipelined, captureFrameToBuffer, getCapturePerfSummary, DrawElementVerificationError, } from "./frameCapture.js";
...
export function shouldDisableBrowserPoolForParallelWorker({ parallel, platform, deviceScaleFactor, headlessShellPath, }) {
    return Boolean(parallel && platform === "linux" && headlessShellPath && (deviceScaleFactor ?? 1) <= 1);
}
export function selectWorkerDiagnostics(lines, maxLines = MAX_WORKER_DIAGNOSTIC_LINES) {
    return lines
        .filter((line) => /\[(FrameCapture:ERROR|Browser:ERROR|Browser:PAGEERROR|Browser:REQUESTFAILED|Browser:HTTP\d{3})\]/.test(line))
        .slice(-maxLines);
}
 ...
 */
export function synthesizeSilentWorkerExitError(result, expectedFrames) {
    return (`worker ${result.workerId} exited without terminal error string ` +
 ...
 */
export function flagSilentWorkerExits(results) {
...
export function formatWorkerFailure(result) {
    const errorText = result.error && result.error.length > 0
        ? result.error
...
export function computeWorkerSizing(totalFrames, requested, config) {
    ...
    const heapBasedWorkers = Math.max(1, Math.floor((heapLimitMb - HEAP_RESERVED_MB) / HEAP_PER_WORKER_MB));
    ...
    const finish = (workers, boundBy, effectiveMaxWorkers) => ({
        workers,
        boundBy,
        effectiveMaxWorkers,
        ...base,
    ...
    if (requested !== undefined) {
        return finish(Math.max(MIN_WORKERS, Math.min(ABSOLUTE_MAX_WORKERS, requested)), "explicit", ABSOLUTE_MAX_WORKERS);
    }
    ...
    const optimalBound = optimal === cpuBasedWorkers ? "cpu" : optimal === memoryBasedWorkers ? "memory" : "frames";
    ...
    let boundBy = finalWorkers === optimal
        ? optimalBound
 ...
 */
export function distributeFramesInterleaved(totalFrames, workerCount, workDir, rangeStart = 0) {
    const tasks = [];
...
function raceAgainstAbort(promise, signal) {
    if (!signal)
        return promise;
    if (signal.aborted)
        return Promise.reject(new Error("Parallel worker cancelled"));
...
async function captureFrameRange(session, task, captureOptions, signal, onFrameCaptured, onFrameBuffer) {
    ...
    if (onFrameBuffer && session.workerEncodeEnabled) {
        ...
        for (let i = task.startFrame; i < task.endFrame; i += stride) {
            ...
            const { encodeResult } = await raceAgainstAbort(captureFrameToBufferPipelined(session, i - outputOffset, time), signal);
            ...
            if (prev) {
                if (dbg && prev.idx < task.startFrame + dbgWin) {
                    console.log(`[par:w${task.workerId}] +${Date.now() - dbgT0}ms drain ${prev.idx} await-encode`);
                }
                const buf = await prev.encodeResult;
                if (dbg && prev.idx < task.startFrame + dbgWin) {
                    console.log(`[par:w${task.workerId}] +${Date.now() - dbgT0}ms drain ${prev.idx} encoded ${buf.length}B`);
                }
                ...
                framesCaptured++;
                if (onFrameCaptured)
                    onFrameCaptured(task.workerId, prev.idx);
            }
    ...
    for (let i = task.startFrame; i < task.endFrame; i += stride) {
        ...
        if (onFrameBuffer) {
            const { buffer } = await raceAgainstAbort(captureFrameToBuffer(session, fileFrameIdx, time), signal);
 ...
 */
export function selectVerifySampleIndicesForTask(sampleIndices, task) {
    const stride = task.frameStride ?? 1;
    const selected = [];
    for (const idx of sampleIndices) {
        if (idx < task.startFrame || idx >= task.endFrame)
            continue;
        if ((idx - task.startFrame) % stride !== 0)
            continue;
        selected.push(idx);
    }
...
function assertDiskSampleAboveFloor(db, verifyMinDb, idx, workerId) {
    if (db < verifyMinDb) {
        ...
        throw new DrawElementVerificationError(`drawElement self-verify failed at frame ${idx} (disk path, worker ${workerId}): ` +
            `${db.toFixed(1)}dB < ${verifyMinDb}dB vs pre-injection screenshot`, { kind: "psnr", frameIndex: idx, failedDb: db, verifyThresholdDb: verifyMinDb });
    }
    console.log(`[Parallel] drawElement disk self-verify passed (worker ${workerId}, frame ${idx}, ` +
 ...
 */
export function isFfmpegInfrastructureFailure(err) {
...
async function psnrForDiskSample(framePath, truth, workerId, idx) {
    try {
        ...
        if (isFfmpegInfrastructureFailure(err)) {
            ...
            throw new Error(`[Parallel] drawElement disk self-verify aborted (worker ${workerId}, frame ${idx}): ` +
                ...
                "HYPERFRAMES_FFMPEG_PATH now points at a different binary.", { cause: err });
        }
        console.warn(`[Parallel] drawElement disk self-verify sample skipped (worker ${workerId}, ` +
...
// fallow-ignore-next-line complexity
export async function verifyDiskDrawElementSamples(session, task, streaming) {
    // Streaming capture verifies every sampled frame in the drain guard already.
    if (streaming || session.captureMode !== "drawelement")
        return;
    const truths = session.deVerifyFrames;
    if (!truths || truths.size === 0)
        return;
...
async function executeWorkerTask(task, serverUrl, captureOptions, createBeforeCaptureHook, signal, onFrameCaptured, onFrameBuffer, config, parallel, onFailure) {
    ...
    const workerConfig = needsSeparateBrowsers
        ? { ...config, enableBrowserPool: false }
        : config;
    try {
        session = await createCaptureSession(serverUrl, task.outputDir, captureOptions, createBeforeCaptureHook(), workerConfig);
        ...
        logParDebug(() => `[par:w${task.workerId}] init done (mode=${session?.captureMode} workerEncode=${session?.workerEncodeEnabled === true})`);
        framesCaptured = await captureFrameRange(session, task, captureOptions, signal, onFrameCaptured, onFrameBuffer);
 ...
 */
export function resolveParallelDeVerifySamples(callerValue, workerCount) {
...
export async function executeParallelCapture(serverUrl, workDir, tasks, captureOptions, createBeforeCaptureHook, signal, onProgress, onFrameBuffer, config) {
    ...
    const totalFrames = tasks.reduce((sum, t) => sum + Math.ceil((t.endFrame - t.startFrame) / (t.frameStride ?? 1)), 0);
    ...
    const deVerifySamples = resolveParallelDeVerifySamples(captureOptions.deVerifySamples, tasks.length);
    const workerCaptureOptions = deVerifySamples === captureOptions.deVerifySamples
        ? captureOptions
    ...
    let firstFatalFailure;
    const onFailure = (failure) => {
        if (firstFatalFailure || !isFatalCaptureFailure(failure))
            return;
        firstFatalFailure = failure;
        peerController.abort(failure);
    };
    const results = await Promise.all(tasks.map((task) => executeWorkerTask(task, serverUrl, workerCaptureOptions, createBeforeCaptureHook, workerSignal, onFrameCaptured, onFrameBuffer, config, parallel, onFailure)));
