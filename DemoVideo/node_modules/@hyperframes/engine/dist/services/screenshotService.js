...
import { HF_COLOR_GRADING_CANVAS_ID_PREFIX, MEDIA_RENDER_ID_ATTR, MEDIA_VISUAL_STYLE_PROPERTIES, RENDER_FRAME_ID_PREFIX, RENDER_FRAME_ID_SUFFIX, renderFrameIdForRenderId, } from "@hyperframes/core";
...
export function shouldDefaultCaptureBeyondViewport(browserVersion, platform = process.platform) {
 ...
 */
export async function probeBeginFrameLiveness(page, timeoutMs, 
...
// further BeginFrames.
frameTimeTicks, intervalMs) {
    const client = await getCdpSession(page);
    const params = {};
    if (typeof frameTimeTicks === "number")
 ...
 */
export async function pageContentExceedsCaptureHeight(page, requestedHeight) {
...
export async function applyDomLayerMask(page, showIds, extraHideIds) {
    ...
    (args) => {
        ...
        const renderFrameId = (id) => `${args.renderFramePrefix}${id}${args.renderFrameSuffix}`;
        ...
        const rememberAndHideElement = (el) => {
            if (el.getAttribute(args.hiddenAttr) !== "1") {
                ...
                const prevPriority = typeof el.style.getPropertyPriority === "function"
        ...
        for (const id of args.show) {
            ...
            if (el)
                rememberHiddenTimedDescendants(el);
 ...
 */
export async function removeDomLayerMask(page, _extraHideIds) {
 ...
 */
export async function ensureRenderFrameSiblings(page) {
    await page.evaluate((prefix, suffix) => {
        for (const video of Array.from(document.querySelectorAll("video[data-start]"))) {
            const next = video.nextElementSibling;
            if (next !== null && next.classList.contains("__render_frame__"))
                continue;
    ...
    }, RENDER_FRAME_ID_PREFIX, RENDER_FRAME_ID_SUFFIX);
}
/**
...
export async function injectVideoFramesBatch(page, updates) {
    ...
    // fallow-ignore-next-line complexity
    async (items, visualProperties, colorGradingSourceHiddenAttr) => {
        const injectedIds = [];
        ...
        const isVisualAncestorHidden = (el) => {
        ...
        for (const item of items) {
            const video = (window.__hfMediaEl?.(item.videoId) ??
                document.getElementById(item.videoId));
            if (!video)
                continue;
    ...
    updates.map((update) => ({ ...update, frameId: renderFrameIdForRenderId(update.videoId) })), [...MEDIA_VISUAL_STYLE_PROPERTIES], COLOR_GRADING_SOURCE_HIDDEN_ATTR);
}
export async function syncVideoFrameVisibility(page, activeVideoIds) {
    await page.evaluate(
    // fallow-ignore-next-line complexity
    (ids, colorGradingSourceHiddenAttr) => {
        ...
        const setColorGradingVisibility = window.__hf?.colorGrading?.setSourceVisibility;
        ...
        for (const video of videos) {
            ...
            if (visible) {
                ...
                video.style.setProperty("pointer-events", "none", "important");
                if (hasImg) {
                    if (video.hasAttribute(colorGradingSourceHiddenAttr))
                        img.style.opacity = "1";
                    img.style.visibility = "visible";
                }
            }
    ...
    }, activeVideoIds, COLOR_GRADING_SOURCE_HIDDEN_ATTR);
}
