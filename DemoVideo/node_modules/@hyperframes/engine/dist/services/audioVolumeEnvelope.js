 ...
 */
export function createEnvelopeWalker(keyframes, trackStart, baseVolume) {
    ...
    return (time) => {
        for (;;) {
            ...
            if (segment >= envelope.length - 2 || !next || time < next.time)
                break;
            segment += 1;
        }
...
function scaleSamples(buffer, layout, gainAt) {
    ...
    const scaleOne = float
        ? (at, gain) => buffer.writeFloatLE(buffer.readFloatLE(at) * gain, at)
        : (at, gain) => {
