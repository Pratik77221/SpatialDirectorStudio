 ...
 */
function findReferenceTargetEl(doc, refId) {
 ...
 * to 0 (never NaN).
 */
export function resolveReferencedStart(doc, el, startCache, visiting) {
    const cached = startCache.get(el);
    if (cached !== undefined)
        return cached;
    if (visiting.has(el))
        return 0; // cycle guard (A -> B -> A)
    visiting.add(el);
    try {
        ...
        const resolved = targetDuration != null && targetDuration > 0
 ...
 */
function resolveReferencedDuration(doc, el, startCache, visiting) {
