...
function redactFfprobeInput(stderr, filePath) {
    ...
    if (inputBasename)
        redacted = redacted.split(inputBasename).join("[input]");
    ...
    if (!redacted.startsWith("[input]")) {
        for (let offset = 1; offset < filePath.length; offset += 1) {
            ...
            if (suffix.length < 4 || !redacted.startsWith(suffix))
                continue;
...
function sanitizeFfprobeDiagnostic(stderr, filePath) {
    ...
    if (redacted.length <= FFPROBE_ERROR_MAX_CHARS)
        return redacted;
...
async function runFfprobe(filePath, argsWithoutInput, signal, stdoutOptions) {
    ...
    stdout += decoder.end();
    if (stdoutTruncated) {
        throw new Error(`[FFmpeg] ffprobe output exceeded ${stdoutMaxChars} characters; refusing to parse a truncated result.`);
    }
    ...
    if (outcome.reason !== "exit" || outcome.exitCode !== 0) {
        ...
        throw new Error(`[FFmpeg] ffprobe ${outcome.reason} with code ${outcome.exitCode}: ${diagnostic}`);
    }
    return stdout;
}
...
const finalVideoFrameTimestampCache = new Map();
const finalVideoFrameTimestampSignalCaches = new WeakMap();
const audioMetadataCache = new Map();
const mediaProbeOutputCache = new Map();
const mediaProbeOutputSignalCaches = new WeakMap();
...
async function hasAvifFileBrand(filePath) {
    try {
        const chunks = [];
        ...
        if (avifBrands.has(bytes.toString("ascii", 8, 12)))
            return true;
        for (let offset = 16; offset + 4 <= boxSize; offset += 4) {
            if (avifBrands.has(bytes.toString("ascii", offset, offset + 4)))
                return true;
        }
        return false;
    }
...
async function isStillImageVisual(output, filePath) {
    const formatNames = (output.format.format_name ?? "")
        .split(",")
        .map((name) => name.trim().toLowerCase())
        .filter(Boolean);
    return (formatNames.some((name) => STILL_IMAGE_DEMUXERS.has(name)) || (await hasAvifFileBrand(filePath)));
}
function isPngImageProbe(output) {
    const formatNames = (output.format.format_name ?? "")
        .split(",")
        .map((name) => name.trim().toLowerCase());
    return (formatNames.includes("png_pipe") ||
        output.streams.some((stream) => stream.codec_type === "video" && stream.codec_name?.toLowerCase() === "png"));
}
function mediaFileIdentity(filePath) {
    try {
...
async function probeMediaOutput(filePath, signal) {
    let cache = mediaProbeOutputCache;
    if (signal) {
        cache = mediaProbeOutputSignalCaches.get(signal) ?? new Map();
    ...
    const promise = runFfprobe(filePath, ["-print_format", "json", "-show_format", "-show_streams"], signal).then(parseProbeJson);
    if (identity !== null) {
...
async function readFileRange(filePath, start, length, signal) {
    const chunks = [];
...
async function hasCompletePngStructure(filePath, signal) {
    try {
        ...
        while (offset + 12 <= fileSize) {
            ...
            if (chunkType === "IHDR")
                seenHeader = chunkLength === 13 && offset === 8;
            if (chunkType === "IDAT")
                seenImageData = true;
            if (chunkType === "IEND")
                return seenHeader && seenImageData && chunkLength === 0;
            offset = chunkEnd;
        }
        return false;
    }
 ...
 */
export async function probeMediaProfile(filePath, options) {
    try {
        ...
        const hasMovingVideoStream = videoStreams.some((stream) => stream.disposition?.attached_pic !== 1);
        ...
        if (error instanceof StructurallyIncompletePngError)
            throw error;
...
function crc32Fallback(data, seed) {
...
function chunkCrc32(chunkType, chunkData) {
    ...
    if (nativeCrc32)
        return nativeCrc32(chunkData, nativeCrc32(typeBytes));
...
export function extractPngMetadataFromBuffer(buf) {
    ...
    while (pos + 12 <= buf.length) {
        ...
        if (chunkCrc32(chunkType, chunkData) !== chunkCrc)
            return null;
        ...
        if (width > 0 && height > 0 && colorSpaceFromCicp !== null)
            break;
        if (chunkType === "IEND")
            break;
        pos += 12 + chunkLen;
 ...
 */
export function readTagCI(tags, name) {
    if (!tags)
        return "";
...
export function parseFrameRate(frameRateStr) {
    ...
    const strict = (part) => part === undefined || part.trim() === "" ? NaN : Number(part.trim());
...
export async function extractMediaMetadata(filePath) {
    ...
    const probePromise = (async () => {
        ...
        const frames = Number.isFinite(parsedNbFrames) && parsedNbFrames > 0 ? parsedNbFrames : undefined;
        return {
 ...
 */
export async function extractFinalVideoFrameTimestamp(filePath, metadata, signal) {
    ...
    const probePromise = (async () => {
        if (!(videoDurationSeconds > 0) || !Number.isFinite(videoDurationSeconds)) {
            throw new Error(`[FFmpeg] Cannot locate final video frame for invalid duration ${String(videoDurationSeconds)}`);
        }
        ...
        const parseFinalTimestamp = (stdout) => stdout
            .split("\n")
        ...
        const probe = async (readInterval) => {
            ...
            if (readInterval)
                args.splice(2, 0, "-read_intervals", readInterval);
        ...
        const timestamp = (await probe(`${intervalStart}%${streamEnd}`)) ?? (await probe( /* full scan */));
...
export async function extractAudioMetadata(filePath, options) {
