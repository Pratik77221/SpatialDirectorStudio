 ...
 */
export function isExternalFfmpegInterruption(result) {
...
function formatWindowsFfmpegExit(exitCode) {
    if (process.platform !== "win32" || exitCode === null)
        return undefined;
    if (exitCode === 3221225781 || exitCode === -1073741515) {
        const ffmpegPath = getFfmpegBinary();
        return (`[FFmpeg] Windows could not start "${ffmpegPath}": ` +
    ...
    if (exitCode === 3221225595 || exitCode === -1073741701) {
        return ("[FFmpeg] Windows could not start ffmpeg.exe (STATUS_INVALID_IMAGE_FORMAT). " +
    ...
    if (exitCode === 3221225794 || exitCode === -1073741502) {
        return ("[FFmpeg] Windows failed while initializing ffmpeg.exe. " +
