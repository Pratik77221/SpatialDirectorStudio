...
export const NOT_MEDIA_PAYLOAD = "NOT_MEDIA_PAYLOAD";
...
export class NotMediaPayloadError extends Error {
    ...
    retryable = false;
    elementFingerprints;
    constructor(elementFingerprints) {
        ...
        super(`${elementFingerprints.length} media source(s) are text documents ` +
...
export function fingerprintElementId(elementId) {
...
function byteOrderMarkLength(head) {
 ...
 */
export async function isNotMediaPayload(filePath) {
    try {
 ...
 */
export async function assertMediaPayload(filePath, elementId) {
    if (await isNotMediaPayload(filePath)) {
        throw new NotMediaPayloadError([fingerprintElementId(elementId)]);
    }
}
//# sourceMappingURL=notMediaPayload.js.map