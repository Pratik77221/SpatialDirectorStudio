...
export function assertConfiguredFfmpegBinariesExist() {
    const ffmpegPath = process.env[FFMPEG_PATH_ENV]?.trim();
    if (ffmpegPath && !existsSync(ffmpegPath)) {
        throw new Error(`[FFmpeg] FFmpeg binary not found at ${FFMPEG_PATH_ENV}="${ffmpegPath}". ` +
    ...
    if (ffprobePath && !existsSync(ffprobePath)) {
        throw new Error(`[FFmpeg] FFprobe binary not found at ${FFPROBE_PATH_ENV}="${ffprobePath}". ` +
...
//# sourceMappingURL=ffmpegBinaries.js.map