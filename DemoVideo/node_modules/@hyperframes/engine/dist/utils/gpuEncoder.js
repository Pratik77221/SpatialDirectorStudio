...
export function getCompiledGpuEncoders(ffmpegEncodersStdout) {
    return GPU_ENCODER_CANDIDATES.filter((encoder) => ffmpegEncodersStdout.includes(H264_ENCODER_BY_GPU[encoder]));
}
export async function selectUsableGpuEncoder(candidates, isUsable) {
    const results = await Promise.all(candidates.map(async (encoder) => {
        try {
...
export function getProbeArgs(encoder) {
    const args = [
        "-hide_banner",
        "-loglevel",
        "error",
...
function logGpuProbeFailure(encoder, result) {
    if (!isGpuProbeDebugEnabled())
        return;
