...
function assertEngineConfigNumber(config, field, min, integer = false) {
    const value = config[field];
    if (typeof value !== "number" ||
        !Number.isFinite(value) ||
        value < min ||
        (integer && !Number.isInteger(value))) {
        throw new Error(`Engine config ${field} must be a ${integer ? "finite integer" : "finite number"} >= ${min}`);
    }
}
function assertPositiveEngineConfigNumber(config, field) {
    const value = config[field];
    if (typeof value !== "number" || !Number.isFinite(value) || value <= 0) {
        throw new Error(`Engine config ${field} must be a finite number > 0`);
    }
}
function assertEngineConfigEnum(config, field, values) {
    if (!values.includes(config[field]))
        throw new Error(`Engine config ${field} is invalid`);
}
function validateRequiredEngineConfigFields(config) {
...
function validateEngineConfigScalars(config) {
...
function validateEngineConfigParallelism(config) {
    ...
    for (const field of ["minParallelFrames", "largeRenderThreshold"]) {
        assertEngineConfigNumber(config, field, 0, true);
    }
}
function validateEngineConfigVp9(config) {
    if (typeof config.vp9CpuUsed !== "number" ||
...
function validateEngineConfigRuntime(config) {
...
function validateEngineConfigHdr(config) {
    const { hdr } = config;
    if (hdr !== false &&
        (!isPlainObject(hdr) ||
...
function validateOptionalEngineConfigFields(config) {
    for (const field of ["chromePath", "runtimeManifestPath", "extractCacheDir"]) {
 ...
 */
export function validateEngineConfigSnapshot(value) {
    if (!isPlainObject(value))
        throw new Error("Engine config snapshot must be a plain object");
 ...
 * for tests.
 */
export function scaleProtocolTimeoutForComposition(baseTimeoutMs, dims) {
...
export function shouldAutoDisableStreamingEncodeOnWin32Compound(opts) {
    if (opts.userExplicitlySet)
        return false;
    if (opts.platform !== "win32")
        return false;
    if (!opts.softwareGpuForced)
        return false;
 ...
 */
export const EXTRACT_CACHE_DIR_DISABLED_ALIASES = ["off", "none", "false", "0"];
/**
 ...
 */
export function defaultExtractCacheDir() {
 ...
 */
export function resolveExtractCacheDir(env = process.env) {
...
function resolveExtractCacheDirFromEnv(env) {
    const raw = env("HYPERFRAMES_EXTRACT_CACHE_DIR");
...
function memoryAdaptiveCacheLimit() {
    ...
    if (total < 4096)
        return 32;
    if (total <= LOW_MEMORY_TOTAL_MB_THRESHOLD)
        return 64;
 ...
 */
function isDrawElementPlatform(platform) {
...
export function resolveDefaultDrawElement(args) {
    if (!args.useDrawElement)
        return false;
    if (args.explicitOptIn)
        return true;
    if (!isDrawElementPlatform(args.platform) || args.browserGpuMode === "software")
        return false;
...
export function explainDrawElementDisabled(args) {
    ...
    if (!isDrawElementPlatform(args.platform))
        return "unsupported_platform";
    if (args.browserGpuMode === "software")
        return "software_gpu";
    if (!args.workerEncode)
        return "worker_encode_off";
    return "disabled";
}
export function resolveConfig(overrides) {
    ...
    const envVp9CpuUsed = () => {
        ...
        if (raw === undefined || raw === "")
            return DEFAULT_CONFIG.vp9CpuUsed;
    ...
    const fromEnv = {
        ...
        enableDrawElementWorkerEncode: envBool("HF_DE_WORKER_ENCODE", DEFAULT_CONFIG.enableDrawElementWorkerEncode),
        ...
        frameDataUriCacheLimit: Math.max(32, envNum("PRODUCER_FRAME_DATA_URI_CACHE_LIMIT", memoryAdaptiveCacheLimit())),
        frameDataUriCacheBytesLimitMb: Math.max(64, envNum("PRODUCER_FRAME_DATA_URI_CACHE_BYTES_MB", memoryAdaptiveCacheBytesMb())),
    ...
    if (merged.browserGpuMode === "software" &&
    ...
    const streamingEncodeUserExplicitlySet = env("PRODUCER_ENABLE_STREAMING_ENCODE") !== undefined ||
    ...
    if (merged.enableStreamingEncode &&
        ...
        console.error("[hyperframes] Windows compound-workaround auto-detected — disabling streaming-encode " +
 ...
 */
export function shouldClampToScreenshotForConcreteGpu(resolvedGpuMode, currentForceScreenshot, env = process.env, opts = {}) {
    if (currentForceScreenshot)
        return false;
    if (resolvedGpuMode !== "software")
        return false;
    if (opts.programmaticOptOut)
        return false;
 ...
 */
export function applyConcreteGpuScreenshotClamp(currentForceScreenshot, resolvedGpuMode, cfg, env = process.env) {
    return (currentForceScreenshot ||
